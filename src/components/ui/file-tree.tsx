...
export const FileTree = React.forwardRef<HTMLDivElement, FileTreeProps>(
  ({ items, onItemClick, onFolderToggle, expandedFolders = new Set(), className }, ref) => {
    ...
    const renderTreeItem = React.useCallback((item: FileTreeItem) => {
      ...
      // Fleet spacing formula: 8px base + (level * 16px)
      ...
      if (item.type === 'folder') {
        return (
          <div 
            key={item.id}
            className="flex items-center gap-1 w-full min-w-0 cursor-pointer hover:bg-[var(--fleet-listItem-background-hovered)] px-3 py-1 mx-1.5 rounded transition-colors duration-75" 
